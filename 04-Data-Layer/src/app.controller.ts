    ...
    export class AppController {
      ...
      async home(@Req() req: Sess, @Res() res: Response, @Query('page') page = '1', @Query('q') q = '') {
        ...
        if (false) {
          ...
          const posts = rows.map(p => ({ id: null, title: p.title, authorName: p.author.displayName, excerpt: p.content.length > 140 ? p.content.slice(0,140)+'…' : p.content }));
        ...
        const posts = rows.map(p => ({ id: null, title: p.title, authorName: p.author.displayName, excerpt: p.content.length > 140 ? p.content.slice(0,140)+'…' : p.content })); return this.render(res, req, 'home', { title: 'Latest stories', heading: 'Latest stories', lead: 'Stories from Circuit Daily.', posts });
      }
